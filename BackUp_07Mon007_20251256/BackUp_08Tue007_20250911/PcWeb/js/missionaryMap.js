...
window.MissionaryMap = class MissionaryMap {
    ...
    updateMissionaryNamePopupPositions() {
        ...
        if (this.state.fixedCountry) {
            ...
            // 노회별 보기 모드 (간단한 업데이트)
            circularPopups.forEach(popup => {
                const missionaryName = popup.textContent;
                const missionary = this.state.missionaries.find(m => m.name === missionaryName);
                if (missionary) {
                    const latlng = this.constants.LATLNGS[missionary.country];
                    if (latlng) {
                        const point = this.map.latLngToContainerPoint(latlng);
                        // 원형 배치를 위한 간단한 오프셋 적용
                        popup.style.left = `${point.x + 5}px`;
    ...
    showFloatingListPopup(country, missionaryNames, latlng) {
        ...
        // 팝업 위치 보정 (화면 밖으로 나가지 않도록, 최소 여백 20px)
        const popupRect = popupElement.getBoundingClientRect();
        const mapRect = this.elements.mapContainer.getBoundingClientRect();
        let left = point.x - popupRect.width / 2;
        let top = point.y - 100;
        // 화면 기준 좌표로 변환
        let absLeft = mapRect.left + left;
        let absTop = mapRect.top + top;
        // 우측/하단 오버플로우 보정 (최소 여백 20px)
        if (absLeft + popupRect.width > window.innerWidth - 20) {
            left -= (absLeft + popupRect.width - (window.innerWidth - 20));
        }
        if (absLeft < 20) {
            left += (20 - absLeft);
        }
        if (absTop + popupRect.height > window.innerHeight - 20) {
            top -= (absTop + popupRect.height - (window.innerHeight - 20));
        }
        if (absTop < 20) {
            top += (20 - absTop);
        }
    ...
    forceStopPrayerRotation() {
        if (window.prayerPopupManager) {
            ...
            // 자동 재시작 타이머가 있다면 정리
            if (window.prayerPopupManager.autoRestartTimer) {
                clearTimeout(window.prayerPopupManager.autoRestartTimer);
                window.prayerPopupManager.autoRestartTimer = null;
            }
            
            console.log('기도 팝업 강제 중지 및 팝업 즉시 닫기 완료');
        }
    }
    ...
    forceResumePrayerRotation() {
        if (window.prayerPopupManager) {
            // 강제 일시정지 상태 해제
            window.prayerPopupManager.forcePaused = false;
            // 기도 팝업 재개
            window.prayerPopupManager.forceResume();
            ...
            console.log('기도 팝업 강제 재개 완료 및 즉시 표시');
        }
    }
